...
d[conf][sid][opt]=true;}}},unset(conf,sid,opt){return this.set(conf,sid,opt,null);},get_first(conf,type,opt){let sid=null;this.sections(conf,type,s=>{sid??=s['.name'];});return this.get(conf,sid,opt);},set_first(conf,type,opt,val){let sid=null;this.sections(conf,type,s=>{sid??=s['.name'];});return this.set(conf,sid,opt,val);},unset_first(conf,type,opt){return this.set_first(conf,type,opt,null);},move(conf,sid1,sid2,after){const sa=this.sections(conf);let s1=null;let s2=null;sid1=this.resolveSID(conf,sid1);sid2=this.resolveSID(conf,sid2);for(let i=0;i<sa.length;i++){if(sa[i]['.name']!=sid1)
